...
from hgp.likelihood import OneHotOrdinalDistribution


class TestOneHotOrdinalDistribution(tf.test.TestCase):
    def setUp(self) -> None:
        self.params = tf.convert_to_tensor(value=[[4., 2., 1.], [2., 5., -1.]])
    ...
    def test_log_prob_uses_prob(self) -> None:
        y = tf.convert_to_tensor(value=[[0., 0., 1.], [0., 1., 0.]])
    ...
    def test_most_likely_category(self) -> None:
        params = tf.convert_to_tensor(value=[[5., 0.]])
        distribution = OneHotOrdinalDistribution(params)
        category_1 = tf.convert_to_tensor(value=[[1., 0]])
        category_2 = tf.convert_to_tensor(value=[[0, 1.]])
    ...
    def test_prob_handles_single_3D_parameters(self) -> None:
        params = tf.convert_to_tensor(value=[[[4., 2., 1.], [2., 5., -1.]]])
        distribution = OneHotOrdinalDistribution(params)
        y = tf.convert_to_tensor(value=[[0., 0., 1.], [0., 1., 0.]])
    ...
    def test_prob_gives_same_result_for_2D_and_3D_parameters(self) -> None:
        params_3D = tf.convert_to_tensor(value=[[[4., 2., 1.], [2., 5., -1.]]])
