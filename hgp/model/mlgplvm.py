...
class MLGPLVM(MLGP):
    ...
    def _create_qx(self, x: np.ndarray) -> Tuple[tf.Tensor, tf.Tensor]:
        with tf.compat.v1.variable_scope("qx"):
            mean = tf.compat.v1.get_variable("mean", shape=[self.num_data, self.x_dim],
                                             initializer=tf.compat.v1.constant_initializer(x))
            log_var = tf.compat.v1.get_variable("log_var", shape=[self.num_data, self.x_dim],
    ...
    def _kl_qx_px(self) -> tf.Tensor:
        ...
        kl_sum = tf.reduce_sum(input_tensor=kl, axis=[0, 1], name="kl_sum")
        return kl_sum
    ...
    def _compute_x_noise(self) -> tf.Tensor:
        qx_mean, qx_var = self._get_or_subsample_qx()
        num_data = tf.shape(input=qx_mean)[0]
    ...
    def _compute_a(self, x_samples: tf.Tensor) -> tf.Tensor:
        ...
        a = tf.transpose(a=a_transposed, perm=[1, 0, 2], name="a")
        return a
    ...
    def impute(self) -> tf.Tensor:
        ...
        imputation = tf.compat.v1.where(nan_mask, mode, y, name="imputation")
        return imputation
