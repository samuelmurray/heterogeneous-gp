...
class BatchMLGPLVM(MLGPLVM):
    def __init__(self, y: np.ndarray, xdim: int, *,
        ...
        self.batch_indices = tf.placeholder(shape=self.batch_size, dtype=tf.int32)
        self.qx_mean_batch = tf.gather(self.qx_mean, self.batch_indices)
        self.qx_var_batch = tf.gather(self.qx_var, self.batch_indices)
        self.y_batch = tf.gather(self.y, self.batch_indices)
