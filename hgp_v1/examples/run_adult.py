...
import hgp_v1
from hgp_v1.data import Unsupervised
from hgp_v1.model import BatchMLGPLVM, VAEMLGPLVM
import hgp_v1.util

ROOT_PATH = os.path.dirname(hgp_v1.__file__)
    ...
    with tf.Session() as sess:
        ...
        for i in range(n_iter):
            ...
            if i % n_print == 0:
                ...
                imputation_error = hgp_v1.util.imputation_error(imputation, y_noisy, y, likelihood)
...
if __name__ == "__main__":
    ...
    for i in range(1, 11):
        ...
        y, likelihood, _ = Unsupervised.make_adult(num_data)
        if num_data is None:
            num_data = y.shape[0]

        idx_to_remove = np.loadtxt(
            os.path.join(ROOT_PATH, os.pardir, "util", NAME, f"Missing20_{i}.csv"),
            delimiter=",")
        idx_to_remove -= 1  # The files are 1-index for some reason
        y_noisy = hgp_v1.util.remove_data(y, idx_to_remove, likelihood)

        model_name = f"VAEMLGPLVM_{i}"
        with tf.name_scope(model_name):
            print(f"Creating model {model_name}...")
            kernel = hgp_v1.kernel.ARDRBF(x_dim=latent_dim)
            num_inducing = 100
            num_hidden = 100
            num_layers = 1
    ...
    print(f"Numerical error over all 10 runs: "
          f"{np.mean(numerical_errors)} +- {np.std(numerical_errors)}")
    print(f"Nominal error over all 10 runs: "
          f"{np.mean(nominal_errors)} +- {np.std(nominal_errors)}")
