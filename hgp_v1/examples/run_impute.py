...
from hgp_v1.likelihood import LikelihoodWrapper
    ...
    with tf.Session() as sess:
        ...
        for i in range(n_iter):
            ...
            if i % args.print_interval == 0:
                ...
                numerical_error, nominal_error = hgp_v1.util.imputation_error(imputation,
                                                                              y_noisy,
                                                                              y_true,
                                                                              model.likelihood)
...
def create_batch_mlgplvm(y_noisy: np.ndarray, likelihood: LikelihoodWrapper) -> BatchMLGPLVM:
    kernel = hgp_v1.kernel.ARDRBF(x_dim=args.latent_dim)
...
def remove_data_randomly(y: np.ndarray, likelihood: LikelihoodWrapper) -> np.ndarray:
    ...
    y_noisy = hgp_v1.util.remove_data_randomly(y, fraction_missing, likelihood)
    return y_noisy
